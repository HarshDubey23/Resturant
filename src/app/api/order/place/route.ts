...
export async function POST(req: Request) {
        ...
        try {
                ...
                if (order) {
                        ...
                        try {
                                ...
                                if (restaurantID && deductedInventory) {
                                        await restoreInventoryForOrder(restaurantID, deductedInventory).catch((e) => captureError(e, { context: "inventory-restore-after-merge-save-failed", orderId: order._id.toString() }));
                                }
                ...
                const tip = Number.isFinite(tipAmount) && tipAmount > 0
