...
export async function POST(req: Request) {
        try {
                        ...
                        .map((p) => {
                                ...
                                const productRef =
                                        typeof raw === "object" && raw !== null && "_id" in raw
                                                ? (raw as { _id: mongoose.Types.ObjectId })._id
                                                : (raw as mongoose.Types.ObjectId);
                ...
                if (restaurantID && inventoryProducts.length) {
                        await restoreInventoryForOrder(restaurantID, inventoryProducts).catch((e: unknown) => captureError(e, { context: "inventory-restore-on-cancel", orderId: orderIdStr }));
                }
                ...
                if (order.loyaltyAwarded && restaurantID && customer) {
                        ...
                        if (pointsToClawback > 0) {
                                await clawbackPointsAtomic(restaurantID, customer, pointsToClawback).catch((e: unknown) => captureError(e, { context: "loyalty-clawback-failed", orderId: orderIdStr }));
                        }
                }
