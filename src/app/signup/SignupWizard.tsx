...
export default function SignupWizard() {
	...
	const validateStep = (n: number): boolean => {
		...
		if (n === 4) {
			for (const item of state.menuItems) {
				if (!item.name.trim()) {
					e[`menu-${item.id}`] = "Name required";
					break;
				}
				if (!item.price || Number(item.price) <= 0) {
					e[`menu-${item.id}`] = "Valid price required";
					break;
				}
			}
		}
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-slate-50 via-violet-50/30 to-slate-50 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950">
			...
			{/* Top bar */}
			<header className="sticky top-0 z-30 backdrop-blur-xl bg-white/70 dark:bg-zinc-950/70 border-b border-slate-100">
				<div className="mx-auto max-w-5xl px-4 sm:px-6 h-16 flex items-center justify-between">
					...
					<a href="/dashboard" className="text-sm text-muted-foreground hover:text-foreground transition-colors">
						Already a customer? <span className="font-semibold text-violet-600">Sign in</span>
					</a>
				</div>
			</header>
			...
			{!success && (
				<div className="mx-auto max-w-5xl px-4 sm:px-6 py-8 sm:py-12">
					...
					<div className="text-center mb-10">
						...
						<p className="mt-2 text-muted-foreground">Everything you set up here goes live instantly. You can change it later from your dashboard.</p>
					</div>
					...
					<div className="mt-10 flex items-center justify-between gap-3">
						...
						{step < 9 ? (
							<button
								onClick={next}
								disabled={submitting}
								className="inline-flex items-center gap-2 px-6 h-11 rounded-xl text-sm font-semibold bg-violet-600 text-white hover:bg-violet-700 shadow-soft disabled:opacity-50 transition-all">
								Continue
					...
					{errors.submit && <div className="mt-6 rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">{errors.submit}</div>}
				</div>
			)}
				...
				{progress && (
					<motion.div
						...
						<div className="max-w-md w-full bg-card rounded-2xl border border-border shadow-2xl p-8 text-center">
							<div className="mx-auto w-16 h-16 rounded-2xl bg-violet-600/10 flex items-center justify-center mb-5">
			...
			{STEP_DEFS.map((s, i) => {
				...
				return (
					<div key={s.id} className="flex items-center flex-1 last:flex-none">
						<button onClick={() => onJump(s.id)} disabled={s.id >= currentStep} className="flex items-center gap-2 group disabled:cursor-default">
							<div
								className={`h-9 w-9 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
									isActive
										? "bg-violet-600 text-white shadow-soft scale-110"
										: isDone
							...
							<span
								className={`text-xs font-medium hidden md:block ${isActive ? "text-foreground" : isDone ? "text-violet-600" : "text-muted-foreground"}`}>
								{s.short}
							</span>
						</button>
						{i < STEP_DEFS.length - 1 && (
							<div className={`h-0.5 flex-1 mx-2 rounded-full transition-all ${s.id < currentStep ? "bg-violet-600" : "bg-border"}`} />
						)}
					</div>
				);
			})}
...
function BrandStep({
	...
	slugStatus: { state: "idle" | "checking" | "available" | "taken"; message?: string };
}) {
		...
		<div className="grid lg:grid-cols-5 gap-6">
			<div className="lg:col-span-3 space-y-6">
				...
				<Field
					...
					hint={slugStatus.state === "available" ? undefined : "This is your unique link — yourdomain.com/your-restaurant"}>
					<div
						className={`flex items-stretch rounded-xl border bg-card overflow-hidden ${errors.restaurantID || slugStatus.state === "taken" ? "border-destructive" : slugStatus.state === "available" ? "border-emerald-400" : "border-border"}`}>
						...
						<div className="p-4 -mt-8 relative">
							<div className="h-16 w-16 rounded-2xl bg-card border-4 border-card overflow-hidden shadow-md mb-3">
								{state.logoUrl ? (
									...
									<div className="h-full w-full bg-violet-600/10 flex items-center justify-center text-violet-600 font-bold text-xl">
				...
				<div className="grid grid-cols-2 gap-3">
					...
					{state.photos.length < 4 && (
						<button
							...
							className="aspect-square rounded-xl border-2 border-dashed border-border hover:border-primary hover:bg-violet-600/5 transition-colors flex flex-col items-center justify-center gap-1 text-muted-foreground">
						...
						{THEME_PRESETS.map((p) => {
							...
							return (
								<button
									...
									className={`flex flex-col items-center gap-1.5 p-2 rounded-xl border-2 transition-all ${active ? "border-primary bg-violet-600/5" : "border-transparent hover:border-border"}`}>
...
function MenuStep({
	...
	const updateItem = (id: string, patch: Partial<MenuItemDraft>) => {
		update(
			"menuItems",
			state.menuItems.map((i) => (i.id === id ? { ...i, ...patch } : i)),
		);
	};
	const removeItem = (id: string) =>
		update(
			"menuItems",
			state.menuItems.filter((i) => i.id !== id),
		);
	...
	return (
		<div className="space-y-6">
			<StepHeader
				title="Menu items"
				subtitle="Add your dishes now or skip and do it from the dashboard later. Each item can have a photo, spice level, and kitchen station."
			/>
			...
			<div className="rounded-xl border border-border bg-card p-4">
				...
				<div className="flex flex-wrap gap-2 mb-3">
					{state.categories.map((c) => (
						<span
							key={c}
							className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-violet-600/10 text-violet-600 border border-primary/20">
							{c}
							<button
								onClick={() =>
									update(
				...
				<div className="flex gap-2">
					...
					/>
					<button onClick={addCategory} className="px-4 py-2 rounded-lg bg-violet-600 text-white text-sm font-medium hover:bg-violet-700">
						Add
					</button>
				</div>
			</div>
			...
			<button
				onClick={addMenuItem}
				className="w-full py-4 rounded-xl border-2 border-dashed border-border hover:border-primary hover:bg-violet-600/5 transition-colors text-sm font-medium text-muted-foreground hover:text-violet-600">
				+ Add menu item
			</button>
		...
		<div className={`rounded-xl border bg-card p-4 ${error ? "border-destructive" : "border-border"}`}>
			<div className="flex items-start gap-4">
				...
				<button
					...
					className="h-20 w-20 rounded-xl border-2 border-dashed border-border hover:border-primary hover:bg-violet-600/5 flex items-center justify-center overflow-hidden flex-shrink-0">
					{item.image ? (
				...
				<div className="flex-1 grid sm:grid-cols-2 gap-3">
					...
					<select
						value={item.veg}
						onChange={(e) => onChange({ veg: e.target.value as MenuItemDraft["veg"] })}
						className="px-3 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
						{VEG_OPTIONS.map((o) => (
					...
					<select
						value={item.foodType}
						onChange={(e) => onChange({ foodType: e.target.value as MenuItemDraft["foodType"] })}
						className="px-3 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
					...
					<select
						value={item.station}
						onChange={(e) => onChange({ station: e.target.value as MenuItemDraft["station"] })}
						className="px-3 py-2 rounded-lg border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
				...
				<button
					onClick={onRemove}
					className="h-8 w-8 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive flex items-center justify-center flex-shrink-0">
					<X className="h-4 w-4" />
				</button>
			</div>
		...
		<div className="grid lg:grid-cols-2 gap-6">
			<div className="space-y-6">
				...
				<Field label="Number of tables">
					<div className="flex items-center gap-3">
						<button
							onClick={() => update("tableCount", Math.max(1, state.tableCount - 1))}
							className="h-11 w-11 rounded-xl border border-border bg-card text-xl font-bold hover:bg-muted/50">
							−
						</button>
						<input
							type="number"
						...
						/>
						<button
							onClick={() => update("tableCount", Math.min(100, state.tableCount + 1))}
							className="h-11 w-11 rounded-xl border border-border bg-card text-xl font-bold hover:bg-muted/50">
							+
						</button>
					</div>
				...
				<div className="rounded-xl bg-violet-600/5 border border-primary/20 p-4 text-sm">
					<p className="font-semibold text-violet-600 mb-1">What happens next?</p>
					<p className="text-muted-foreground">
						After you launch, your dashboard's <strong>Settings → Tables</strong> tab will show every QR code ready to download or print. Place one on each
						table.
					</p>
				</div>
			</div>
			...
			<div className="space-y-6">
				...
				<div className="rounded-xl border border-border bg-card p-4 space-y-3 text-sm">
					...
					<div className="flex items-start gap-3">
						<div className="h-8 w-8 rounded-lg bg-violet-600/10 flex items-center justify-center text-violet-600 font-bold text-xs flex-shrink-0">A</div>
						<div>
...
function AIKeysStep({ state, update }: { state: WizardState; update: <K extends keyof WizardState>(k: K, v: WizardState[K]) => void }) {
	...
	return (
		<div className="space-y-6">
			<StepHeader
				title="AI provider keys (optional)"
				subtitle="Bring your own keys for the Jarvis AI assistant. If you skip this, we use our shared pool — just slower at peak times."
			/>

			<div className="rounded-xl bg-violet-600/5 border border-primary/20 p-4 text-sm">
				<p className="font-semibold text-violet-600 mb-1">Multi-provider failover</p>
				<p className="text-muted-foreground">
					We try Groq first → Cerebras → Gemini → SiliconFlow → HuggingFace. If one is down or rate-limited, the next takes over automatically. Keys are
				...
				{providers.map((p) => (
					<div key={p.key} className="rounded-xl border border-border bg-card p-4">
						<div className="flex items-center justify-between mb-2">
							...
							<a href={`https://${p.url}`} target="_blank" rel="noreferrer" className="text-xs text-violet-600 hover:underline">
								Get key →
							</a>
						</div>
						<input
	...
	return (
		<div className="space-y-6">
					...
					<div className="absolute bottom-4 left-4 right-4 flex items-end gap-3">
						<div className="h-16 w-16 rounded-2xl border-4 border-card bg-card overflow-hidden shadow-lg">
							{state.logoUrl ? (
								...
								<div className="h-full w-full bg-violet-600/10 flex items-center justify-center text-violet-600 font-bold text-2xl">
									{state.restaurantName.charAt(0).toUpperCase() || "?"}
								</div>
							)}
						</div>
			...
			<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
				{stats.map((s) => (
					<div key={s.label} className="rounded-xl border border-border bg-card p-4 text-center">
						<div className="text-2xl font-black text-violet-600">{s.value}</div>
			...
			<div className="grid sm:grid-cols-2 gap-3">
				...
				<ReviewItem
					label="Theme"
					value={`HSL ${state.themeH}° ${state.themeS}% ${state.themeL}%`}
					swatch={`hsl(${state.themeH} ${state.themeS}% ${state.themeL}%)`}
				/>
				...
				<ReviewItem
					...
					value={
						[
							state.aiGroq && "Groq",
							state.aiCerebras && "Cerebras",
							state.aiGoogle && "Gemini",
							state.aiSiliconFlow && "SiliconFlow",
							state.aiHuggingFace && "HuggingFace",
						]
							.filter(Boolean)
							.join(", ") || "Using shared pool"
					}
				/>
			</div>
				...
				<ol className="space-y-2 text-sm text-muted-foreground">
					{[
						...
						<li key={i} className="flex gap-3">
							<span className="h-5 w-5 rounded-full bg-violet-600 text-white text-xs font-bold flex items-center justify-center flex-shrink-0">
								{i + 1}
							</span>
							{s}
						</li>
	...
	return (
		<div>
			...
			<button
				...
				className={`relative w-full ${aspect} rounded-xl border-2 border-dashed ${err ? "border-destructive" : "border-border"} hover:border-primary hover:bg-violet-600/5 transition-colors flex items-center justify-center overflow-hidden`}>
				{value ? (
...
function SuccessScreen({
	...
	return (
		<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mx-auto max-w-4xl px-4 sm:px-6 py-12">
			<div className="text-center mb-10">
				...
				<div className="mt-4 inline-flex items-center gap-2 rounded-full bg-muted px-4 py-1.5 text-xs">
					...
					<a href={success.restaurantUrl} target="_blank" rel="noopener noreferrer" className="font-mono font-semibold text-violet-600 hover:underline">
			...
			<div className="rounded-2xl border border-border bg-card p-6 mb-8">
				...
				<ol className="space-y-2 text-sm text-muted-foreground">
					<li>
						<span className="font-semibold text-foreground">1.</span> Print the QR codes and place one on each table.
					</li>
					<li>
						<span className="font-semibold text-foreground">2.</span> Open your dashboard to add more menu items, edit prices, or change your theme.
					</li>
					<li>
						<span className="font-semibold text-foreground">3.</span> Scan a QR code with your phone to test the customer flow end-to-end.
					</li>
					<li>
						<span className="font-semibold text-foreground">4.</span> The kitchen display at <span className="font-mono">/kitchen</span> shows live orders —
						bookmark it on a tablet.
					</li>
				</ol>
			</div>
			...
			<div className="flex flex-col sm:flex-row gap-3 justify-center">
				<button
					...
					className="inline-flex items-center justify-center gap-2 px-6 h-12 rounded-xl text-sm font-semibold bg-violet-600 text-white hover:bg-violet-700 shadow-soft transition-all">
