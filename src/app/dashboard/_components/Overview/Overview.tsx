	...
	return (
		<div className="space-y-6">
			...
			<div className="flex items-end justify-between flex-wrap gap-3">
				<div>
					<h2 className="text-lg font-bold tracking-tight text-slate-900">Welcome{profile?.name ? `, ${profile.name.split(" ")[0]}` : ""} 👋</h2>
					<p className="text-xs text-slate-500 mt-0.5 leading-relaxed">Here&apos;s what&apos;s happening today</p>
				</div>
			...
			<div className="grid lg:grid-cols-3 gap-6">
				{/* Revenue chart */}
				<div className="lg:col-span-2 rounded-2xl border border-slate-100 bg-white shadow-soft p-5">
					<div className="flex items-center justify-between mb-4">
						<div>
							<h3 className="text-sm font-semibold text-slate-900 tracking-tight">Revenue · last 7 days</h3>
							<p className="text-xs text-slate-500 mt-0.5 leading-relaxed">{formatCurrency(weekRevenue, currency)} total</p>
						</div>
						<Link
							href="/dashboard?tab=analytics"
							className="text-xs text-violet-600 hover:text-violet-700 inline-flex items-center gap-1 transition-all duration-200">
					...
					{data?.dailyRevenue && data.dailyRevenue.length > 0 ? (
						<ResponsiveContainer width="100%" height={220}>
							<AreaChart data={data.dailyRevenue} margin={{ top: 10, right: 0, left: -10, bottom: 0 }}>
								...
								/>
								<Area type="monotone" dataKey="revenue" stroke="hsl(263 70% 50%)" strokeWidth={2.5} fill="url(#revFill)" />
							</AreaChart>
						</ResponsiveContainer>
					) : (
						<div className="flex flex-col items-center justify-center h-[220px] text-xs text-slate-500 gap-2">
				...
				{/* Quick actions */}
				<div className="rounded-2xl border border-slate-100 bg-white shadow-soft p-5">
					<h3 className="text-sm font-semibold text-slate-900 tracking-tight mb-3">Quick actions</h3>
					<div className="space-y-2">
						...
						<QuickAction
							...
							icon={ChefHat}
							title="Active orders"
							subtitle={`${data?.activeOrders ?? 0} in progress`}
						/>
			...
			<div className="grid lg:grid-cols-3 gap-6">
				<div className="lg:col-span-2 rounded-2xl border border-slate-100 bg-white shadow-soft p-5">
					<div className="flex items-center justify-between mb-3">
						<h3 className="text-sm font-semibold text-slate-900 tracking-tight">Recent orders</h3>
						<Link href="/dashboard?tab=orders" className="text-xs text-violet-600 hover:text-violet-700 transition-all duration-200">
							View all
						</Link>
					</div>
							...
							{data.recentOrders.map((order) => (
								<div key={order._id} className="flex items-center justify-between py-2.5 text-xs">
									<div className="flex items-center gap-3 min-w-0">
										<span className="font-mono text-slate-500 shrink-0">#{order._id.slice(-6)}</span>
										<span className="font-medium text-slate-900">Table {order.table}</span>
										<span className="text-slate-500">{order.products?.length ?? 0} items</span>
									</div>
									<div className="flex items-center gap-3 shrink-0">
										...
										<span className="font-semibold text-slate-900">{formatCurrency((order.orderTotal ?? 0) + (order.taxTotal ?? 0), currency)}</span>
									</div>
								</div>
							))}
						</div>
				...
				<div className="rounded-2xl border border-slate-100 bg-white shadow-soft p-5">
					<div className="flex items-center gap-2 mb-3">
						...
						<h3 className="text-sm font-semibold text-slate-900 tracking-tight">Top items · recent</h3>
					</div>
					{(() => {
						...
						const max = top[0]?.[1] ?? 1;
						if (top.length === 0) {
							return <div className="text-center py-6 text-xs text-slate-500">No items ordered yet</div>;
						}
						return (
								...
								{top.map(([name, count]) => (
									<div key={name} className="text-xs">
										...
										<div className="h-1.5 rounded-full bg-slate-100 overflow-hidden">
											<div className="h-full rounded-full bg-violet-600" style={{ width: `${(count / max) * 100}%` }} />
										</div>
									</div>
								))}
							</div>
...
function KpiCard({
	...
	return (
		<motion.div
			...
			transition={{ delay }}
			className={`relative overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-soft p-5`}>
			<div className={`absolute inset-0 bg-gradient-to-br ${gradient} opacity-60`} />
			<div className="relative z-10">
				<div className="flex items-center justify-between mb-3">
					<div className={`flex h-8 w-8 items-center justify-center rounded-lg ${iconBg} ${pulse ? "animate-pulse" : ""}`}>{icon}</div>
					{delta !== null && (
						<span
				...
				<p className="text-xs text-slate-500 mb-0.5 leading-relaxed">{label}</p>
...
function QuickAction({ href, icon: Icon, title, subtitle }: { href: string; icon: React.ComponentType<{ className?: string }>; title: string; subtitle: string }) {
	return (
		<Link href={href} className="flex items-center gap-3 p-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 transition-all duration-200 ease-out text-sm group">
			<span className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-600/10 text-violet-600">
