...
import { AlertTriangle, Download, FileJson, Loader2, RefreshCw, ShieldCheck, Sparkles } from "lucide-react";
	...
	return (
		<div className="space-y-4 p-4">
			<div>
				...
				<p className="text-sm text-muted-foreground">GSTR-1 / GSTR-3B JSON exports, NIC e-invoice status and the tamper-proof audit chain.</p>
				{!gstEnabled && (
						...
						<CardContent className="space-y-3">
							<div className="flex items-center justify-between rounded-lg border bg-card p-3">
								<div>
									...
									<p className="text-xs text-muted-foreground">
										{einvoiceEnabled ? "Enabled — IRN generation is active." : "Disabled — IRN generation runs in mock mode."}
									</p>
								</div>
								<Badge variant={einvoiceEnabled ? "default" : "secondary"}>{einvoiceEnabled ? "Live" : "Mock"}</Badge>
							</div>
							...
							<div className="rounded-lg border bg-muted/30 p-3 text-xs text-muted-foreground space-y-1">
								...
								<ol className="ml-4 list-decimal space-y-1">
									...
									<li>
										Set <code>EINVOICE_ENABLED=true</code>, <code>NIC_ASN</code>, <code>NIC_USER</code>, <code>NIC_PASSWORD</code>,{" "}
										<code>NIC_GSTIN</code> and <code>NIC_ENV=prod</code> in your environment.
									</li>
									<li>
										Enable e-invoice in Business settings (toggles <code>profile.settings.einvoiceEnabled</code>).
									</li>
							...
							<p className="text-xs text-muted-foreground">
								See <code>docs/EINVOICE_GO_LIVE.md</code> for the full checklist. When <code>EINVOICE_ENABLED</code> is not <code>true</code>, the NIC
								client returns mock IRNs so the order/invoice UI is fully testable end-to-end without burning live NIC quota.
							</p>
						</CardContent>
					</Card>
				</TabsContent>
		...
		<div className="space-y-4">
			<Card>
				<CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-end sm:justify-between">
					<div className="space-y-1.5">
						...
						<Input id="gst-month" type="month" value={month} onChange={(e) => setMonth(e.target.value || currentMonth())} className="w-[200px]" />
					</div>
			...
			{reconciliation && (
				<motion.div
					...
					{reconciliation.matched ? <ShieldCheck className="h-5 w-5 shrink-0" /> : <AlertTriangle className="h-5 w-5 shrink-0" />}
					<div className="flex-1">
						<p className="font-semibold">
							{reconciliation.matched ? "Reconciled — invoices match audit chain" : `Discrepancy of ${reconciliation.discrepancy} detected`}
						</p>
