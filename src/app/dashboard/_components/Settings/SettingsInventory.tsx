...
import { ChevronLeft, ChevronRight, Loader2, Package, PackageOpen, Plus, Trash2 } from "lucide-react";
	...
	return (
		<div className="space-y-4 p-4">
			<div>
				...
				<p className="text-sm text-muted-foreground">Stock journals, GRN, wastage, physical counts, variance & suppliers.</p>
			</div>
			...
			<Dialog open={modalOpen} onOpenChange={setModalOpen}>
				<DialogContent>
					...
					<div className="space-y-3">
						...
						<div className="grid grid-cols-2 gap-3">
							...
							<div className="space-y-1.5">
								...
								<Select
									value={form.unit}
									onValueChange={(v) => {
										if (v) setForm({ ...form, unit: v as Unit });
									}}>
									<SelectTrigger>
										<SelectValue />
									</SelectTrigger>
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-1.5">
								<Label htmlFor="item-open" className="text-xs">
									Opening Stock
								</Label>
								<Input
									id="item-open"
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-1.5">
								<Label htmlFor="item-reorder" className="text-xs">
									Reorder Level
								</Label>
								<Input
									id="item-reorder"
							...
							<div className="space-y-1.5">
								<Label htmlFor="item-reorder-qty" className="text-xs">
									Reorder Qty
								</Label>
								<Input
									id="item-reorder-qty"
						...
						<div className="space-y-1.5">
							<Label htmlFor="item-supplier" className="text-xs">
								Supplier
							</Label>
							<Input
								id="item-supplier"
						...
						<>
							<div className="space-y-1.5">
								...
								<Select
									...
									onValueChange={(v) => {
										if (v) setForm({ ...form, inventoryId: v });
									}}>
									<SelectTrigger>
							...
							<div className="grid grid-cols-2 gap-3">
								...
								<div className="space-y-1.5">
									...
									<Select
										...
										onValueChange={(v) => {
											if (v) setForm({ ...form, reasonCode: v });
										}}>
										<SelectTrigger>
											<SelectValue />
										</SelectTrigger>
...
function PhysicalCountTab() {
	const [_items, setItems] = useState<StockItem[]>([]);
	const [loading, setLoading] = useState(true);
	const [saving, setSaving] = useState(false);
								...
								{form.items.map((row: CountForm["items"][number], i: number) => {
									...
									return (
										<TableRow key={row.inventoryId}>
											...
											<TableCell
												className={`text-right tabular-nums ${delta < 0 ? "text-destructive font-semibold" : delta > 0 ? "text-emerald-600 dark:text-emerald-400 font-semibold" : ""}`}>
												{delta > 0 ? "+" : ""}
												{delta.toFixed(2)}
											</TableCell>
										</TableRow>
			...
			<Dialog open={modalOpen} onOpenChange={setModalOpen}>
				<DialogContent>
					...
					<div className="space-y-3">
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-1.5">
								<Label htmlFor="sup-phone" className="text-xs">
									Phone
								</Label>
								<Input id="sup-phone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
							</div>
							<div className="space-y-1.5">
								<Label htmlFor="sup-gstin" className="text-xs">
									GSTIN
								</Label>
						...
						<div className="space-y-1.5">
							<Label htmlFor="sup-items" className="text-xs">
