...
export default function ShiftZReport({ data, onClose }: ShiftZReportProps) {
	...
	if (!shift || !metrics) {
		return <div className="rounded-xl border bg-destructive/10 p-4 text-sm text-destructive">Unable to render Z report — missing data.</div>;
	}
			...
			{flagged && (
				<motion.div
					...
					<p className="text-xs opacity-90">
						Shortfall of {formatCurrency(Math.max(0, metrics.expectedCash - metrics.countedCash))} has been reported to n8n via <code>cash.shift_short</code>.
						This shift cannot be reopened.
					</p>
				</motion.div>
			)}
