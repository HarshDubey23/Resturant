...
export default function CashierBilling() {
        ...
        }, [items, table, discount, searchOpen, recallOpen, shortcutsOpen, payMode]);
        ...
        const onTender = handleTenderSubmit(async (values) => {
                ...
                try {
                        ...
                        toast.success(
                                payMode === "cash"
                                        ? `Paid • Change ${formatCurrency(change, currency)}`
                                        : `UPI link ready`,
                        );
                        clearBill();
                        setPayMode(null);
        ...
        return (
                <div className="flex h-[calc(100vh-7rem)] flex-col gap-3">
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                        {filteredMenus.length === 0 ? (
                                                                <div className="col-span-2 py-10 text-center text-xs text-muted-foreground">
                                                                        No menu items. Add items in Settings → Menu.
                                                                </div>
                                                        ) : (
                                                                filteredMenus.map((m) => (
                                                                        <motion.button
                        ...
                        {/* Modals */}
                        <ShiftOpen open={locked || shiftOpenOpen} onOpened={() => { setShiftOpenOpen(false); refetchShift(); }} dismissable={shiftOpenOpen && !locked} onCancel={() => setShiftOpenOpen(false)} />
                                                                ...
                                                                heldBills.map((h) => (
                                                                        <div key={h.id} className="flex items-center justify-between rounded-lg border p-2 text-sm">
                                                                                <div>
                                                                                        ...
                                                                                        <p className="text-xs text-muted-foreground">
                                                                                                {h.items.length} items • {formatCurrency(h.items.reduce((s, i) => s + i.price * i.qty, 0), currency)}
                                                                                        </p>
                                                                                </div>
                                                                                <div className="flex items-center gap-1">
                                                ...
                                                <div className="space-y-3">
                                                        {upiId ? (
                                                                <>
                                                                        ...
                                                                        <Button className="w-full" loading={processing} onClick={() => onTender({ tendered: grandTotal })}>
                                                                                Mark paid
                                                                        </Button>
                                                                </>
                                                        ) : (
