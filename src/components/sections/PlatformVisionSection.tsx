...
import { Brain, CalendarClock, Languages, LayoutDashboard, MessageSquare, Phone, QrCode, Shield, ShoppingCart, Sparkles, Star, Utensils } from "lucide-react";
...
const VISION_FEATURES = [
	...
	},
	{
		...
		accent: "from-violet-600/15 to-indigo-500/15",
	...
	},
	{
		...
		accent: "from-violet-500/15 to-amber-500/15",
		ring: "ring-violet-500/20",
	},
	{
		icon: MessageSquare,
...
export default function PlatformVisionSection() {
	return (
		<section id="platform-vision" className="relative py-28 sm:py-36 overflow-hidden bg-slate-50">
			{/* Background */}
			<div className="absolute inset-0 bg-mesh pointer-events-none" />
			<div className="absolute top-0 left-1/3 w-[500px] h-[500px] bg-violet-600/8 rounded-full blur-[120px]" />
			...
			<div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
					...
					{VISION_FEATURES.map((f, i) => {
						...
						return (
							<motion.div
								...
								className="group relative overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-soft p-6 transition-all duration-200 ease-out hover:shadow-soft-hover hover:-translate-y-0.5">
								...
								<div
									className={`absolute inset-0 rounded-2xl ring-1 ring-inset ${f.ring} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
								/>

								<div className="relative z-10">
									<div className="flex items-start justify-between mb-4">
										<div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-600/10 border border-violet-600/20 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
											<Icon className="h-6 w-6 text-violet-600" />
										</div>
										<span className="text-[10px] font-bold uppercase tracking-wide px-2 py-1 rounded-full bg-violet-600/10 text-violet-600 border border-violet-600/20">
											{f.tag}
										</span>
									</div>
									<h3 className="text-base font-bold text-slate-900 mb-2 tracking-tight">{f.name}</h3>
				...
				<motion.div
					...
					className="mt-16 flex flex-col sm:flex-row items-center justify-center gap-4 p-8 rounded-3xl bg-gradient-to-br from-violet-600/10 via-fuchsia-500/5 to-transparent border border-violet-600/20">
					<div className="text-center sm:text-left">
						<p className="text-xl font-bold text-slate-900">All twelve systems. One restaurant. One dashboard.</p>
						<p className="text-sm text-slate-500 mt-1 leading-relaxed">
							Activate the ones you want — skip the ones you don't. No code, no integrations to maintain.
						</p>
					</div>
					<a
						href="/signup"
						className="inline-flex items-center gap-2 px-6 h-12 rounded-xl text-sm font-semibold bg-violet-600 text-white hover:bg-violet-700 shadow-soft hover:shadow-soft-hover hover:-translate-y-0.5 transition-all duration-200 ease-out whitespace-nowrap active:scale-[0.98]">
						Get started free
