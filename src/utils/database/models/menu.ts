...
MenuSchema.pre("save", async function () {
        ...
        const validCategories = account?.profile?.categories?.map((c: string) => c.toLowerCase());
        if (validCategories?.length && !validCategories.includes(this.category)) {
                throw new Error(`The menu item category does not exist. Got '${this.category}', expected one of: ${validCategories.join(", ")}`);
        }
});
